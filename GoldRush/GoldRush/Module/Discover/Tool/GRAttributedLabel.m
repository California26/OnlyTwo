...
#import "GRAttributedLabel.h"

#import <QuartzCore/QuartzCore.h>
#import <Availability.h>
#import <objc/runtime.h>

#define kGRLineBreakWordWrapTextWidthScalingFactor (M_PI / M_E)

static CGFloat const GRFLOAT_MAX = 100000;

NSString * const kGRStrikeOutAttributeName = @"GRStrikeOutAttribute";
NSString * const kGRBackgroundFillColorAttributeName = @"GRBackgroundFillColor";
NSString * const kGRBackgroundFillPaddingAttributeName = @"GRBackgroundFillPadding";
NSString * const kGRBackgroundStrokeColorAttributeName = @"GRBackgroundStrokeColor";
NSString * const kGRBackgroundLineWidthAttributeName = @"GRBackgroundLineWidth";
NSString * const kGRBackgroundCornerRadiusAttributeName = @"GRBackgroundCornerRadius";

const NSTextAlignment GRTextAlignmentLeft = NSTextAlignmentLeft;
const NSTextAlignment GRTextAlignmentCenter = NSTextAlignmentCenter;
const NSTextAlignment GRTextAlignmentRight = NSTextAlignmentRight;
const NSTextAlignment GRTextAlignmentJustified = NSTextAlignmentJustified;
const NSTextAlignment GRTextAlignmentNatural = NSTextAlignmentNatural;

const NSLineBreakMode GRLineBreakByWordWrapping = NSLineBreakByWordWrapping;
const NSLineBreakMode GRLineBreakByCharWrapping = NSLineBreakByCharWrapping;
const NSLineBreakMode GRLineBreakByClipping = NSLineBreakByClipping;
const NSLineBreakMode GRLineBreakByTruncatingHead = NSLineBreakByTruncatingHead;
const NSLineBreakMode GRLineBreakByTruncatingMiddle = NSLineBreakByTruncatingMiddle;
const NSLineBreakMode GRLineBreakByTruncatingTail = NSLineBreakByTruncatingTail;

typedef NSTextAlignment GRTextAlignment;
typedef NSLineBreakMode GRLineBreakMode;


static inline CTTextAlignment CTTextAlignmentFromGRTextAlignment(GRTextAlignment alignment) {
    switch (alignment) {
...
static inline CTLineBreakMode CTLineBreakModeFromGRLineBreakMode(GRLineBreakMode lineBreakMode) {
...
static inline CGFloat GRFlushFactorForTextAlignment(NSTextAlignment textAlignment) {
    switch (textAlignment) {
        case GRTextAlignmentCenter:
            return 0.5f;
        case GRTextAlignmentRight:
            return 1.0f;
        case GRTextAlignmentLeft:
        default:
            return 0.0f;
    }
}

static inline NSDictionary * NSAttributedStringAttributesFromLabel(GRAttributedLabel *label) {
    ...
    if ([NSMutableParagraphStyle class]) {
        ...
        CTTextAlignment alignment = CTTextAlignmentFromGRTextAlignment(label.textAlignment);
        ...
        if (label.numberOfLines == 1) {
            lineBreakMode = CTLineBreakModeFromGRLineBreakMode(label.lineBreakMode);
        }
...
static inline CGSize CTFramesetterSuggestFrameSizeForAttributedStringWithConstraints(CTFramesetterRef framesetter, NSAttributedString *attributedString, CGSize size, NSUInteger numberOfLines) {
    ...
    CGSize constraints = CGSizeMake(size.width, GRFLOAT_MAX);
    
    if (numberOfLines == 1) {
        // If there is one line, the size that fits is the full width of the line
        constraints = CGSizeMake(GRFLOAT_MAX, GRFLOAT_MAX);
    } else if (numberOfLines > 0) {
        // If the line count of the label more than 1, limit the range to size to the number of lines that have been set
        CGMutablePathRef path = CGPathCreateMutable();
        CGPathAddRect(path, NULL, CGRectMake(0.0f, 0.0f, constraints.width, GRFLOAT_MAX));
...
@interface GRAccessibilityElement : UIAccessibilityElement
@property (nonatomic, weak) UIView *superview;
@property (nonatomic, assign) CGRect boundingRect;
@end

@implementation GRAccessibilityElement
...
@property (readwrite, nonatomic, strong) GRAttributedLabelLink *activeLink;
@property (readwrite, nonatomic, strong) NSArray *accessibilityElements;

- (void) longPressGestureDidFire:(UILongPressGestureRecognizer *)sender;
@end

@implementation GRAttributedLabel {
@private
...
- (void)addLink:(GRAttributedLabelLink *)link {
    [self addLinks:@[link]];
}
...
- (void)addLinks:(NSArray *)links {
    ...
    for (GRAttributedLabelLink *link in links) {
        if (link.attributes) {
...
- (GRAttributedLabelLink *)addLinkWithTextCheckingResult:(NSTextCheckingResult *)result
                                  ...
                                  attributes:(NSDictionary *)attributes
{
    ...
    for (NSTextCheckingResult *result in results) {
        ...
        GRAttributedLabelLink *link = [[GRAttributedLabelLink alloc] initWithAttributes:attributes
...
- (GRAttributedLabelLink *)addLinkToURL:(NSURL *)url
                               withRange:(NSRange)range
{
    return [self addLinkWithTextCheckingResult:[NSTextCheckingResult linkCheckingResultWithRange:range URL:url]];
}

- (GRAttributedLabelLink *)addLinkToAddress:(NSDictionary *)addressComponents
                                   withRange:(NSRange)range
{
...
- (GRAttributedLabelLink *)addLinkToPhoneNumber:(NSString *)phoneNumber
                                       withRange:(NSRange)range
{
...
- (GRAttributedLabelLink *)addLinkToDate:(NSDate *)date
                                withRange:(NSRange)range
{
...
- (GRAttributedLabelLink *)addLinkToTransitInformation:(NSDictionary *)components
                                              withRange:(NSRange)range
{
...
- (GRAttributedLabelLink *)linkAtPoint:(CGPoint)point {
    ...
    GRAttributedLabelLink *result = [self linkAtCharacterIndex:[self characterIndexAtPoint:point]];
...
- (GRAttributedLabelLink *)linkAtRadius:(const CGFloat)radius aroundPoint:(CGPoint)point {
    ...
    GRAttributedLabelLink *link = nil;
...
- (GRAttributedLabelLink *)linkAtCharacterIndex:(CFIndex)idx {
...
- (CFIndex)characterIndexAtPoint:(CGPoint)p {
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        CGFloat flushFactor = GRFlushFactorForTextAlignment(self.textAlignment);
                ...
                context:(CGContextRef)c
{
    ...
    CFArrayRef lines = CTFrameGetLines(frame);
    NSInteger numberOfLines = self.numberOfLines > 0 ? MIN(self.numberOfLines, CFArrayGetCount(lines)) : CFArrayGetCount(lines);
    BOOL truncateLastLine = (self.lineBreakMode == GRLineBreakByTruncatingHead || self.lineBreakMode == GRLineBreakByTruncatingMiddle || self.lineBreakMode == GRLineBreakByTruncatingTail);
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        if (lineIndex == numberOfLines - 1 && truncateLastLine) {
            ...
            if (!(lastLineRange.length == 0 && lastLineRange.location == 0) && lastLineRange.location + lastLineRange.length < textRange.location + textRange.length) {
                ...
                GRLineBreakMode lineBreakMode = self.lineBreakMode;
                
                // Multiple lines, only use UILineBreakModeTailTruncation
                if (numberOfLines != 1) {
                    lineBreakMode = GRLineBreakByTruncatingTail;
                }
                
                switch (lineBreakMode) {
                    case GRLineBreakByTruncatingHead:
                        truncationType = kCTLineTruncationStart;
                        break;
                    case GRLineBreakByTruncatingMiddle:
                        truncationType = kCTLineTruncationMiddle;
                        truncationAttributePosition += (lastLineRange.length / 2);
                        break;
                    case GRLineBreakByTruncatingTail:
                    default:
               ...
               context:(CGContextRef)c
{
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            ...
            CGColorRef strokeColor = CGColorRefFromColor([attributes objectForKey:kGRBackgroundStrokeColorAttributeName]);
            CGColorRef fillColor = CGColorRefFromColor([attributes objectForKey:kGRBackgroundFillColorAttributeName]);
            UIEdgeInsets fillPadding = [[attributes objectForKey:kGRBackgroundFillPaddingAttributeName] UIEdgeInsetsValue];
            CGFloat cornerRadius = [[attributes objectForKey:kGRBackgroundCornerRadiusAttributeName] floatValue];
            CGFloat lineWidth = [[attributes objectForKey:kGRBackgroundLineWidthAttributeName] floatValue];
           ...
           context:(CGContextRef)c
{
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            NSDictionary *attributes = (__bridge NSDictionary *)CTRunGetAttributes((__bridge CTRunRef) glyphRun);
            BOOL strikeOut = [[attributes objectForKey:kGRStrikeOutAttributeName] boolValue];
...
- (void)setActiveLink:(GRAttributedLabelLink *)activeLink {
     ...
     limitedToNumberOfLines:(NSInteger)numberOfLines
{
    ...
    if (textSize.height < bounds.size.height) {
        ...
        switch (self.verticalAlignment) {
            case GRAttributedLabelVerticalAlignmentCenter:
                yOffset = CGFloat_floor((bounds.size.height - textSize.height) / 2.0f);
                break;
            case GRAttributedLabelVerticalAlignmentBottom:
                yOffset = bounds.size.height - textSize.height;
                break;
            case GRAttributedLabelVerticalAlignmentTop:
            default:
                break;
        }
...
- (void)drawTextInRect:(CGRect)rect {
    ...
    if (self.adjustsFontSizeToFitWidth && self.numberOfLines > 0) {
        ...
        CGSize maxSize = (self.numberOfLines > 1) ? CGSizeMake(GRFLOAT_MAX, GRFLOAT_MAX) : CGSizeZero;
        
        CGFloat textWidth = [self sizeThatFits:maxSize].width;
        CGFloat availableWidth = self.frame.size.width * self.numberOfLines;
        if (self.numberOfLines > 1 && self.lineBreakMode == GRLineBreakByWordWrapping) {
            textWidth *= kGRLineBreakWordWrapTextWidthScalingFactor;
        }
...
- (NSArray *)accessibilityElements {
    if (!_accessibilityElements) {
        @synchronized(self) {
            ...
            for (GRAttributedLabelLink *link in self.linkModels) {
                ...
                if (accessibilityLabel) {
                    GRAccessibilityElement *linkElement = [[GRAccessibilityElement alloc] initWithAccessibilityContainer:self];
            ...
            GRAccessibilityElement *baseElement = [[GRAccessibilityElement alloc] initWithAccessibilityContainer:self];
...
- (void)longPressGestureDidFire:(UILongPressGestureRecognizer *)sender {
    switch (sender.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            GRAttributedLabelLink *link = [self linkAtPoint:touchPoint];
...
#pragma mark - GRAttributedLabelLink

@implementation GRAttributedLabelLink
...
- (instancetype)initWithAttributesFromLabel:(GRAttributedLabel*)label
                         textCheckingResult:(NSTextCheckingResult *)result {
...
static inline NSDictionary * convertNSAttributedStringAttributesToCTAttributes(NSDictionary *attributes) {
    ...
    NSDictionary *NSToCTAttributeNamesMap = @{
                                              ...
                                              NSBackgroundColorAttributeName: (NSString *)kGRBackgroundFillColorAttributeName,
