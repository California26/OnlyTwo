...
//! Project version number for GRAttributedLabel.
FOUNDATION_EXPORT double GRAttributedLabelVersionNumber;

//! Project version string for GRAttributedLabel.
FOUNDATION_EXPORT const unsigned char GRAttributedLabelVersionString[];

@class GRAttributedLabelLink;

/**
 Vertical alignment for text in a label whose bounds are larger than its text bounds
 */
typedef NS_ENUM(NSInteger, GRAttributedLabelVerticalAlignment) {
    GRAttributedLabelVerticalAlignmentCenter   = 0,
    GRAttributedLabelVerticalAlignmentTop      = 1,
    GRAttributedLabelVerticalAlignmentBottom   = 2,
};

/**
 Determines whether the text to which this attribute applies has a strikeout drawn through itself.
 */
extern NSString * const kGRStrikeOutAttributeName;

/**
 The background fill color. Value must be a `CGColorRef`. Default value is `nil` (no fill).
 */
extern NSString * const kGRBackgroundFillColorAttributeName;

/**
 The padding for the background fill. Value must be a `UIEdgeInsets`. Default value is `UIEdgeInsetsZero` (no padding).
 */
extern NSString * const kGRBackgroundFillPaddingAttributeName;

/**
 The background stroke color. Value must be a `CGColorRef`. Default value is `nil` (no stroke).
 */
extern NSString * const kGRBackgroundStrokeColorAttributeName;

/**
 The background stroke line width. Value must be an `NSNumber`. Default value is `1.0f`.
 */
extern NSString * const kGRBackgroundLineWidthAttributeName;

/**
 The background corner radius. Value must be an `NSNumber`. Default value is `5.0f`.
 */
extern NSString * const kGRBackgroundCornerRadiusAttributeName;

@protocol GRAttributedLabelDelegate;

// Override UILabel @property to accept both NSString and NSAttributedString
@protocol GRAttributedLabel <NSObject>
@property (nonatomic, copy) IBInspectable id text;
@end

IB_DESIGNABLE

@interface GRAttributedLabel : UILabel<GRAttributedLabel, UIGestureRecognizerDelegate>
...
/**
 ...
 @discussion A `GRAttributedLabel` delegate responds to messages sent by tapping on links in the label. You can use the delegate to respond to links referencing a URL, address, phone number, date, or date with a specified time zone and duration.
 */
@property (nonatomic, unsafe_unretained) IBOutlet id <GRAttributedLabelDelegate> delegate;
...
/**
 The vertical text alignment for the label, for when the frame size is greater than the text rect size. The vertical alignment is `GRAttributedLabelVerticalAlignmentCenter` by default.
 */
@property (nonatomic, assign) GRAttributedLabelVerticalAlignment verticalAlignment;
...
/**
 Adds a link. You can customize an individual link's appearance and accessibility value by creating your own @c GRAttributedLabelLink and passing it to this method. The other methods for adding links will use the label's default attributes.
 
 @warning Modifying the link's attribute dictionaries must be done before calling this method.
 
 @param link A @c GRAttributedLabelLink object.
 */
...
/**
 Returns the @c GRAttributedLabelLink at the give point if it exists.
...
/**
 The `GRAttributedLabelDelegate` protocol defines the messages sent to an attributed label delegate when links are tapped. All of the methods of this protocol are optional.
 */
@protocol GRAttributedLabelDelegate <NSObject>
 ...
 */
- (void)attributedLabel:(GRAttributedLabel *)label
...
/**
 ...
 @discussion This method is called if no other delegate method was called, which can occur by either now implementing the method in `GRAttributedLabelDelegate` corresponding to a particular link, or the link was added by passing an instance of a custom `NSTextCheckingResult` subclass into `-addLinkWithTextCheckingResult:`.
...
@interface GRAttributedLabelLink : NSObject <NSCoding>

typedef void (^GRAttributedLabelLinkBlock) (GRAttributedLabel *, GRAttributedLabelLink *);
...
/**
 ...
 @c GRAttributedLabelDelegate tap methods, which will not be called for this link.
 */
@property (nonatomic, copy) GRAttributedLabelLinkBlock linkTapBlock;

/**
 A block called when this link is long-pressed.
 If non-nil, long pressing on this link will call this block instead of the
 @c GRAttributedLabelDelegate long press methods, which will not be called for this link.
 */
@property (nonatomic, copy) GRAttributedLabelLinkBlock linkLongPressBlock;
