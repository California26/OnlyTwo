...
@interface GRNumberBtn () <UITextFieldDelegate>
/** 减按钮*/
...
{
    return [[GRNumberBtn alloc] initWithFrame:frame];
}
#pragma mark - 设置UI子控件
- (void)setupUI
{
    ...
    _textField.text = [NSString stringWithFormat:@"%zd",(long)_minValue];
...
- (void)layoutSubviews
{
    ...
    if (_decreaseHide && _textField.text.integerValue < _minValue) {
        _decreaseBtn.frame = CGRectMake(_width, 0, _height, _height);
    } else {
...
- (void)increase
{
    ...
    if (number <= _maxValue)
    {
        ...
        _textField.text = [NSString stringWithFormat:@"%zd", (long)number];
        
        [self buttonClickCallBackWithIncreaseStatus:YES];
    }
    else
    {
        if (_shakeAnimation) { [self shakeAnimationMethod]; } GRLog(@"已超过最大数量%zd",_maxValue);
        [SVProgressHUD setMinimumDismissTimeInterval:2.0];
        [SVProgressHUD showInfoWithStatus:[NSString stringWithFormat:@"已超过最大数量%zd",(long)_maxValue]];
    }
}
...
- (void)decrease
{
    ...
    else
    {
        ...
        if (_shakeAnimation) { [self shakeAnimationMethod]; } GRLog(@"数量不能小于%ld",(long)_minValue);
        [SVProgressHUD setMinimumDismissTimeInterval:2.0];
        [SVProgressHUD showInfoWithStatus:[NSString stringWithFormat:@"数量不能小于%ld",(long)_minValue]];
    }
}
...
- (void)buttonClickCallBackWithIncreaseStatus:(BOOL)increaseStatus
{
    ...
    if ([_delegate respondsToSelector:@selector(gr_numberButton: number: increaseStatus:)]) {
        [_delegate gr_numberButton:self number:_textField.text.integerValue increaseStatus:increaseStatus];
    }
}
...
- (void)checkTextFieldNumberWithUpdate
{
    ...
    if ([_textField.text gr_isNotBlank] == NO || _textField.text.integerValue < _minValue)
    {
        _textField.text = _decreaseHide ? [NSString stringWithFormat:@"%ld",(long)(minValueString.integerValue-1)]:minValueString;
    }
...
- (void)setDecreaseHide:(BOOL)decreaseHide
{
    ...
    if (decreaseHide)
    {
        if (_textField.text.integerValue <= _minValue)
        {
            _textField.hidden = YES;
            _decreaseBtn.alpha = 0;
            _textField.text = [NSString stringWithFormat:@"%ld",(long)(_minValue-1)];
            _decreaseBtn.frame = CGRectMake(_width-_height, 0, _height, _height);
        }
...
@implementation NSString (GRNumberBtn)
- (BOOL)gr_isNotBlank
{
