...
@interface GRStopButton () <UITextFieldDelegate>
/** 减按钮*/
...
{
    return [[GRStopButton alloc] initWithFrame:frame];
}
#pragma mark - 设置UI子控件
- (void)setupUI
{
...
- (void)increase
{
    ...
    NSInteger number = _textField.text.integerValue + 10;
    ...
    {
        if (_shakeAnimation) { [self shakeAnimationMethod]; } GRLog(@"已超过最大数量%ld",_maxValue);
        [SVProgressHUD setMinimumDismissTimeInterval:2.0];
        [SVProgressHUD showInfoWithStatus:[NSString stringWithFormat:@"已超过最大数量%ld",(long)_maxValue]];
    }
}
...
- (void)decrease
{
    ...
    else
    {
        ...
        if (_shakeAnimation) { [self shakeAnimationMethod]; } GRLog(@"数量不能小于%ld",_minValue);
        [SVProgressHUD setMinimumDismissTimeInterval:2.0];
